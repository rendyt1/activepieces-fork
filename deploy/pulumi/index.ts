...
const efsDirectory = '/data';
const efsRootDirectory = '/app/data';

const stack = pulumi.getStack();
const config = new pulumi.Config();

const apEncryptionKey = config
  .getSecret('apEncryptionKey')
  ?.apply((secretValue) => {
    return (
      secretValue ||
      child_process.execSync('openssl rand -hex 16').toString().trim()
    );
  });
const apJwtSecret = config.getSecret('apJwtSecret')?.apply((secretValue) => {
  return (
    secretValue ||
    child_process.execSync('openssl rand -hex 32').toString().trim()
  );
});
...
if (usePostgres) {
  ...
  if (PERSIST_EFS) {
    efs = new aws.efs.FileSystem(`${stack}-efs`);
    ...
    new aws.efs.MountTarget(`${stack}-mount-target`, {
      fileSystemId: efs.id,
      subnetId: vpc.publicSubnetIds[0],
      securityGroups: [efsSecurityGroup.id],
    });
    new aws.efs.MountTarget(`${stack}-mount-target-az2`, {
      fileSystemId: efs.id,
      subnetId: vpc.publicSubnetIds[1],
      securityGroups: [efsSecurityGroup.id],
    });

    efsAccessPoint = new aws.efs.AccessPoint(`${stack}-efs-accesspoint`, {
      fileSystemId: efs.id,
      rootDirectory: {
        path: efsDirectory,
        creationInfo: {
...
if (useRedis) {
  const redisCluster = new aws.elasticache.Cluster(`${stack}-redis-cluster`, {
    ...
    port: 6379,
    subnetGroupName: new aws.elasticache.SubnetGroup(
      `${stack}-redis-subnet-group`,
      {
...
if (subDomain && domain) {
  ...
  const certificate = new aws.acm.Certificate(`${stack}-cert`, {
...
const environmentVariables = [
  ...
  {
    name: 'AP_CONFIG_PATH',
    value: efsRootDirectory,
  },
];

const taskRole = new aws.iam.Role(`${stack}-fg-taskrole`, {
  name: `${stack}-fg-taskrole`,
...
const fargateService = new awsx.ecs.FargateService(`${stack}-fg`, {
  ...
  taskDefinitionArgs: {
    ...
    container: {
      ...
      mountPoints: [
        {
          sourceVolume: `${stack}-volume`,
          containerPath: efsRootDirectory,
          readOnly: false,
        },
      ],
    },
